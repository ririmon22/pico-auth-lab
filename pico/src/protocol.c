...
static void
packet_clear(Packet *out) {
    out->command[0] = '\0';
    out->context[0] = '\0';
}

static int
packet_parse(char *buf, Packet *out) {
...
protocol_handle_line(char *buf) {
    ...
    } else if (strcmp(packet.command, "SHA3") == 0) {
        sha3_command_handle(packet.context);
    } else {
